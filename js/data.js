    ...
    // Fungsi generik: bisa ke spreadsheet manapun dengan nama sheet atau gid
    function downloadSheet(spreadsheetId, sheetParam, callbackAction) {
        return new Promise(function(resolve) {
            var script  = document.createElement('script');
            var cbName  = 'cb' + Math.random().toString(36).substring(2, 9);
            var done    = false;
            ...
            window[cbName] = function(json) {
                try {
                    ...
                    console.warn('[Portal] Callback error:', e);
                    callbackAction([]);
                }
                finish();
            ...
            script.onerror = function() {
                console.warn('[Portal] Script error: ' + sheetParam);
                callbackAction([]);
                finish();
            };
            ...
            setTimeout(function() {
                if (!done) {
                    console.warn('[Portal] Timeout 8s: ' + sheetParam);
                    callbackAction([]);
                    finish();
                }
            }, 8000);
            ...
            script.src = 'https://docs.google.com/spreadsheets/d/' + spreadsheetId
                + '/gviz/tq?' + sheetParam + '&tqx=responseHandler:' + cbName;
            document.body.appendChild(script);
        });
    }

    // Shortcut untuk spreadsheet utama (nilai, users, plotting) — pakai nama sheet
    function downloadCloudSheet(sheetName, callbackAction) {
        return downloadSheet(SPREADSHEET_ID_UTAMA, 'sheet=' + encodeURIComponent(sheetName), callbackAction);
    }

    // Biodata diambil via Google Apps Script terpisah (URL_APPS_SCRIPT_BIODATA)
    // Apps Script mengembalikan array-of-arrays atau array-of-objects;
    // dinormalisasi ke format gviz { c: [{v:...}] } agar parser yang ada tetap bekerja.
    function downloadBiodataSheet(callbackAction) {
        return fetch(URL_APPS_SCRIPT_BIODATA)
            .then(function(r) { return r.json(); })
            .then(function(data) {
                var rows = Array.isArray(data) ? data
                         : (data.rows || data.data || data.values || []);

                // Normalisasi ke format gviz-compatible yang dipakai parser biodata
                var normalized = rows.map(function(row) {
                    if (!row) return { c: [] };
                    // Sudah dalam format gviz { c: [...] }
                    if (row.c) return row;
                    // Array of arrays
                    if (Array.isArray(row)) {
                        return { c: row.map(function(v) { return { v: v }; }) };
                    }
                    // Array of objects: ambil value dari setiap key
                    return { c: Object.values(row).map(function(v) { return { v: v }; }) };
                });

                callbackAction(normalized);
            })
            .catch(function(e) {
                console.warn('[Portal] Gagal mengunduh biodata dari Apps Script:', e);
                callbackAction([]);
            });
    }
    ...
    // =========================================================================
        ...
        await Promise.all([
            ...
            // 🌟 Biodata diambil dari spreadsheet terpisah (SPREADSHEET_ID_BIODATA)
            downloadBiodataSheet(function(rows) {
                try {
                    dataBiodataGlobal = []; 
