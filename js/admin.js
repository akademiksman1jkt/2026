        ...
        if (total === 0) {
            ...
            slice.forEach(user => {
                ...
                tr.innerHTML = `
                    ...
                    <td style="text-align:center;padding:12px 8px; display:flex; gap:6px; justify-content:center;">
                        <button class="btn-primary" style="padding:6px 8px; background:#0284c7;" title="Reset Password" onclick="resetPasswordCloud('${safeUsername}')"><i class="ri-key-2-line"></i></button>
                        <button class="btn-danger" style="padding:6px 8px;" title="Hapus Akun" onclick="hapusUserCloud('${safeUsername}')"><i class="ri-delete-bin-line"></i></button>
                    </td>
                `;
                frag.appendChild(tr);
            });
    ...
    async function resetPasswordCloud(username) {
        const passwordBaru = prompt(`Reset password untuk akun "${username}".\n\nMasukkan password baru:`);
        if (passwordBaru === null) return;
        if (!passwordBaru.trim()) {
            alert('❌ Password tidak boleh kosong!');
            return;
        }
        const konfirmasi = prompt(`Konfirmasi password baru untuk "${username}".\n\nMasukkan ulang password baru:`);
        if (konfirmasi === null) return;
        if (passwordBaru.trim() !== konfirmasi.trim()) {
            alert('❌ Konfirmasi password tidak cocok. Silakan coba lagi.');
            return;
        }
        if (!confirm(`Reset password akun "${username}"?\n\nPassword baru akan segera aktif.`)) return;

        document.getElementById('loading').style.display = 'flex';
        try {
            await fetch(URL_GOOGLE_APPS_SCRIPT, {
                method: 'POST',
                body: JSON.stringify({ aksi: 'resetPassword', username, passwordBaru: passwordBaru.trim() })
            });
        } catch(e) { /* tetap lanjutkan update lokal */ }

        const userIdx = cloudUsersCache.findIndex(u => u.username === username);
        if (userIdx !== -1) cloudUsersCache[userIdx].pass = passwordBaru.trim();

        renderTabelUserAdmin();
        document.getElementById('loading').style.display = 'none';
        alert(`✅ Password akun "${username}" berhasil direset!`);
    }
