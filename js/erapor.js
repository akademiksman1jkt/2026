    ...
    window.addEventListener('DOMContentLoaded', function() {
        ...
        // Hanya tampilkan lockscreen jika ada sesi tersimpan dan belum dibuka
        let adaSesi = !!localStorage.getItem("session_user_sman1");
        if (!tirai) return;
        if (statusTerbuka === "true" || !adaSesi) {
            tirai.style.display = 'none';
        } else {
            tirai.style.display = 'flex';
        }
    });
...
// ============================================================
// ALIAS FUNGSI — menyambungkan nama tombol di HTML ke fungsi aktual
// ============================================================
function renderInputNilaiGuru()      { muatLembarKerjaErapor(); }
function simpanNilaiEraporCloud()    { simpanKomponenNilaiCloud(); }
function prosesImportExcelErapor()   { prosesImportNilaiExcel(); }
function unduhTemplateExcelErapor()  { unduhTemplateImportErapor(); }

// ============================================================
// KOREKSI & REVISI NILAI — pengajuan perbaikan oleh guru/admin
// ============================================================
function ajukanKoreksiNilai() {
    var identitas  = (document.getElementById('revIdentitas')?.value || '').trim();
    var mapel      = (document.getElementById('revMapel')?.value || '').trim();
    var semester   = (document.getElementById('revSemester')?.value || '').trim();
    var nilaiBaru  = (document.getElementById('revNilaiBaru')?.value || '').trim();
    var fileInput  = document.getElementById('revFotoBukti');

    if (!identitas || !mapel || !semester || !nilaiBaru) {
        alert('⚠️ Mohon lengkapi semua field yang wajib diisi (Identitas, Mapel, Semester, Nilai Baru).');
        return;
    }

    var nilaiNum = parseFloat(nilaiBaru);
    if (isNaN(nilaiNum) || nilaiNum < 0 || nilaiNum > 100) {
        alert('⚠️ Nilai baru harus berupa angka antara 0 – 100.');
        return;
    }

    // Cari data siswa untuk mendapatkan nama
    var siswa = (dataSiswaGlobal || []).find(function(r) {
        return r[0] === identitas || r[1] === identitas;
    });
    var namaSiswa = siswa ? siswa[2] : identitas;
    var mapelInfo = daftarMapel.find(function(m) { return m.kode === mapel; });
    var namaMapel = mapelInfo ? mapelInfo.namaLengkap : mapel;

    function kirimKoreksi(buktiBase64) {
        var listKoreksi = [];
        try { listKoreksi = JSON.parse(localStorage.getItem('db_koreksi_sman1')) || []; } catch(e) {}
        ...
        var entri = {
            ...
            mapel:     namaMapel,
            semester:  semester,
            nilaiBaru: nilaiNum.toFixed(1),
            bukti:     buktiBase64 || 'KOSONG',
            status:    'MENUNGGU PERSETUJUAN',
            pengaju:   sessionUserAktif ? sessionUserAktif.username : '-'
        };

        listKoreksi.push(entri);
        try { localStorage.setItem('db_koreksi_sman1', JSON.stringify(listKoreksi)); } catch(e) {}

        // Kirim ke cloud (non-blocking)
        fetch(URL_GOOGLE_APPS_SCRIPT, {
            method: 'POST',
            body: JSON.stringify({ aksi: 'ajukanKoreksi', data: entri })
        }).catch(function() {});

        // Bersihkan form
        if (document.getElementById('revIdentitas'))  document.getElementById('revIdentitas').value  = '';
        if (document.getElementById('revMapel'))      document.getElementById('revMapel').value      = '';
        if (document.getElementById('revNilaiBaru'))  document.getElementById('revNilaiBaru').value  = '';
        if (fileInput) fileInput.value = '';

        if (typeof renderRiwayatKoreksiSistem === 'function') renderRiwayatKoreksiSistem();
        alert('✅ Pengajuan revisi nilai "' + namaSiswa + '" (' + namaMapel + ' Smt ' + semester + ') berhasil dikirim!\nStatus: Menunggu persetujuan admin.');
    }

    // Proses upload foto jika ada
    if (fileInput && fileInput.files && fileInput.files[0]) {
        var reader = new FileReader();
        reader.onload = function(e) { kirimKoreksi(e.target.result); };
        reader.onerror = function() { kirimKoreksi('KOSONG'); };
